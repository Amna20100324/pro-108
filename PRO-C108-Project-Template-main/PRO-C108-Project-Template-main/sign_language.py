    ...
    if results.multi_hand_landmarks:
        for hand_landmark in results.multi_hand_landmarks:
            ...
            for tip in finger_tips:
                ...
                #writing condition to check if finger if folded i.e checking if finger tip
                # if finger folded changing color to green
                if lm_list[tip].x < lm_lsit[tip -3].x:
            ...
            #checking if all finger are folded
            if all(finger_fold_status):
                #checking if the thumb is up
                if lm_list[thumb_tip].y < lm_lsit[thumb_tip-1].y < lm_list[thumb_tip-2].y:
                    print("LIKE")
